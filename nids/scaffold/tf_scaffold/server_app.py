...
from strategies import scaffold, utilities
...
def main(grid: Grid, context: Context) -> None:
    ...
    log(INFO, "Loading configuration for Scaffold strategy...")
    ...
    global_learning_rate = float(context.run_config["global_learning_rate"])
    ...
    output_folder = output_folder + f"/federated_training_scaffold_{rn_seed}-" + time.strftime("%Y%m%d-%H%M%S") + "/"    
    if os.path.isdir(output_folder) == False:
        os.mkdir(output_folder)
    ...
    with open(output_folder + "summary.txt", "w") as f:    
        ...
        f.write(f"global_learning_rate = {global_learning_rate}\n")
    ...
    # Define and start Scaffold strategy
    strategy = scaffold.Scaffold(client_names,
