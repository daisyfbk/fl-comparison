...
#  
"""Network intrusion detection system with FedAvg strategy using Flower and TensorFlow.
   Server app.
"""
...
from strategies import fedavg4flad, utilities
...
def main(grid: Grid, context: Context) -> None:
    ...
    log(INFO, "Loading configuration for FedAvg strategy...")
    ...
    output_folder = output_folder + f"/federated_training_fedavg_{rn_seed}-" + time.strftime("%Y%m%d-%H%M%S") + "/"    
    if os.path.isdir(output_folder) == False:
        os.mkdir(output_folder)
    ...
    # Define and start FedAvg4Flad strategy.
    # Pass seed for reproducibility of client sampling.
    strategy = fedavg4flad.FedAvg4Flad(client_names,
