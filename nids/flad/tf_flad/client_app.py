...
def train(msg: Message, context: Context):
    ...
    log(INFO, "Received training message from Flad server. Starting local training...")
    client = {}
    ...
    client["epochs"] = int(msg.content["config"]["epochs"])
    client["steps_per_epoch"] = int(msg.content["config"]["steps_per_epoch"])
    ...
    log(INFO, f"Steps_per_epoch={client['steps_per_epoch']}")
    ...
    if client["steps_per_epoch"] > 0:
        client['batch_size'] = max(int(len(client['training'][1]) / client["steps_per_epoch"]), 1)
    else:
...
def evaluate(msg: Message, context: Context):
    ...
    log(INFO, "Received evaluation message from Flad server. Starting local evaluation...")
    client = {}
    client["name"] = str(context.node_config["name"])
    client["data_folder"] = str(context.node_config["data_folder"])
    client['rn_seed'] = int(context.run_config["rn_seed"])
