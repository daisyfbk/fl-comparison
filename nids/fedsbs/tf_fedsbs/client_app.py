...
# Regularization term used for FedProx but also here. 
# It computes the proximal term as the squared L2 norm between
...
def train(msg: Message, context: Context):
    ...
    log(INFO, "Received training message from FedSBS server. Starting local training...")
    client = {}
    ...
    # For FedSBS, we use the received model to compute the loss that will then be 
    # used to compute the fedsbs score.
    X_val, Y_val = client['validation']
    y_prob = np.squeeze(model.predict(X_val, batch_size=2048, verbose=0))
    Y_pred = (y_prob > 0.5).astype(np.int32)
    global_loss = float(tf.keras.losses.binary_crossentropy(Y_val, y_prob).numpy().mean())
    ...
    # Compute the phi value for this client.
    unique, counts = np.unique(client['training'][1], return_counts=True) 
    p = counts/float(len(client['training'][1])) 
    phi = -sum(p * np.log2(p))
    if val_loss < 1:
        phi = 1 - phi

    # Finally compute the fedsbs_score for this client.
    fedsbs_score = -math.log(global_loss) + phi * math.log(val_loss)

    log(INFO, f"Finished local training. Train loss: {train_loss:.4f}, Val loss: {val_loss:.4f}, FedSBS score: {fedsbs_score:.4f}")
...
def evaluate(msg: Message, context: Context):
    ...
    log(INFO, "Received evaluation message from FedSBS server. Starting local evaluation...")
    client = {}
    client["name"] = str(context.node_config["name"])
    client["data_folder"] = str(context.node_config["data_folder"])
    client['rn_seed'] = int(context.run_config["rn_seed"])
...
def info(msg: Message, context: Context) -> Message:
    ...
    log(INFO, f"Received info query from FedSBS server. Responding with client name: {client_name}")
    content = RecordDict({
