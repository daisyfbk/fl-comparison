...
from strategies import dafl, utilities
...
def main(grid: Grid, context: Context) -> None:
    ...
    log(INFO, "Loading configuration for Dafl strategy...")
    ...
    dafl_threshold = float(context.run_config["dafl_threshold"])
    ...
    output_folder = output_folder + f"/federated_training_dafl_{rn_seed}-" + time.strftime("%Y%m%d-%H%M%S") + "/"    
    if os.path.isdir(output_folder) == False:
        os.mkdir(output_folder)
    ...
    with open(output_folder + "summary.txt", "w") as f:
        ...
        f.write(f"dafl_threshold = {dafl_threshold}\n")
    ...
    # Define and start DAFl strategy.
    strategy = dafl.Dafl(client_names=client_names, dafl_threshold=dafl_threshold)
