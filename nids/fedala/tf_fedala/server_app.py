...
from strategies import fedala, utilities
from tf_fedala.ann_models import load_model
...
def main(grid: Grid, context: Context) -> None:
    ...
    log(INFO, "Loading configuration for FedALA strategy...")
    ...
    ala_trainable_layers = int(context.run_config["ala_trainable_layers"])
    ala_sample_ratio = float(context.run_config["ala_sample_ratio"])
    ...
    output_folder = output_folder + f"/federated_training_fedala_{rn_seed}-" + time.strftime("%Y%m%d-%H%M%S") + "/"    
    if os.path.isdir(output_folder) == False:
        os.mkdir(output_folder)
    ...
    with open(output_folder + "summary.txt", "w") as f:
        ...
        f.write(f"ala_trainable_layers = {ala_trainable_layers}\n")
        f.write(f"ala_sample_ratio = {ala_sample_ratio}\n")
    ...
    # Define and start FedALA strategy
    strategy = fedala.FedALA(client_names,
                             fraction_train=fraction_train,
                             rn_seed=rn_seed,
                             ala_trainable_layers=ala_trainable_layers,
                             ala_sample_ratio=ala_sample_ratio)
